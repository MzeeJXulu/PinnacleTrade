...
def _stooq_fetch(pair: str, interval: str, period: str, debug: dict) -> pd.DataFrame:
    """
    Stooq delivers **daily** FX CSV. We'll use it only when others fail.
    URL pattern: https://stooq.com/q/d/l/?s=eurusd&i=d
    """
    ...
    try:
        ...
        if r.status_code != 200 or not r.text or "Date,Open,High,Low,Close,Volume" not in r.text.splitlines()[0]:
            debug["stooq_status"] = f"HTTP {r.status_code}, first_line={r.text.splitlines()[0][:60] if r.text else 'no body'}"
            return pd.DataFrame()
