...
app = FastAPI(title="Trading Signal Service + Dashboard + Webhook", version="3.1.0")
...
def _standardize_ohlcv(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure the expected OHLCV columns exist and are floats; fill Volume if missing."""
    ...
    # fallback attempt with canonical names
    return df[["Open","High","Low","Close","Volume"]].astype(float)


def _yahoo_fetch(pair: str, interval: str, period: str) -> pd.DataFrame:
    """Fetch from Yahoo Finance with a few reliable fallbacks (daily/weekly)."""
    combos = [
...
def _parse_fx_pair_for_av(pair: str) -> tuple[str, str]:
    """
    Convert 'EURUSD=X' -> ('EUR','USD'), 'EURUSD'->('EUR','USD'), 'EUR/USD'->('EUR','USD').
    For non-FX tickers (e.g., AAPL), raise ValueError.
    """
    p = pair.upper().strip()
...
def _alpha_vantage_fetch(pair: str, interval: str, period: str, api_key: str) -> pd.DataFrame:
    """
    Alpha Vantage FX:
      - intraday: function=FX_INTRADAY (interval: 1min/5min/15min/30min/60min)
      - daily:    function=FX_DAILY
    We map:
      1h  -> 60min (direct)
      4h  -> 60min then resample to 4H
      1d  -> daily
      1wk -> daily then resample to weekly
    """
    if not api_key:
        return pd.DataFrame()
    ...
    def _av_intraday(mins: str, outputsize: str = "compact") -> pd.DataFrame:
        ...
        for ts, row in j[key].items():
            records.append({
                ...
                "Volume": 0.0,  # AV FX intraday does not include volume
            })
    ...
    # choose output size based on period
    long = period in {"1y", "2y", "5y", "max", "60d", "90d"}
    outputsize = "full" if long else "compact"

    if interval == "1h":
        return _av_intraday("60", outputsize=outputsize)
    if interval == "4h":
        base = _av_intraday("60", outputsize=outputsize)
        if base.empty:
            return base
        return base.resample("4H").agg({"Open":"first","High":"max","Low":"min","Close":"last","Volume":"sum"}).dropna()
    if interval == "1d":
        return _av_daily(outputsize=outputsize)
    if interval == "1wk":
        base = _av_daily(outputsize=outputsize)
        if base.empty:
            return base
...
def fetch_market_data(pair: str, interval: str, period: str) -> pd.DataFrame:
    """
    Dispatcher that chooses data provider by env:
      DATA_PROVIDER = 'yahoo' (default) or 'alpha_vantage'
      ALPHA_VANTAGE_KEY must be set if DATA_PROVIDER='alpha_vantage'.
    """
    provider = os.getenv("DATA_PROVIDER", "yahoo").strip().lower()
    if provider == "alpha_vantage":
        key = os.getenv("ALPHA_VANTAGE_KEY", "")
        df = _alpha_vantage_fetch(pair, interval, period, key)
        if not df.empty:
            df.attrs["used_provider"] = "alpha_vantage"
            return df
        # If AV fails, fall back to Yahoo
    df = _yahoo_fetch(pair, interval, period)
    if not df.empty:
...
def signal(
    ...
    df: pd.DataFrame = fetch_market_data(pair, interval, period)
    if df is None or df.empty:
        raise HTTPException(status_code=404, detail="No data returned (provider and fallbacks exhausted)")
...
def dashboard(
    ...
    df: pd.DataFrame = fetch_market_data(pair, interval, period)
    if df is None or df.empty:
        return HTMLResponse("<h2>No data returned (provider and fallbacks exhausted)</h2>", status_code=404)
    ...
    <body>
      <div class="card">
        ...
        <div class="footer">
          Tip: For strict intraday reliability, set DATA_PROVIDER=alpha_vantage and ALPHA_VANTAGE_KEY in Render.
        </div>
      </div>
    </body>
    </html>
