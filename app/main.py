...
import os, requests, re, io, csv
import pandas as pd
import yfinance as yf
from app.analysis_engine import compute_indicators, decide_signal

app = FastAPI(title="Trading Signal Service + Dashboard + Webhook", version="3.2.0")
...
def _standardize_ohlcv(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all(k in cols for k in ["open","high","low","close"]):
        ...
        if "volume" in cols:
            data["Volume"] = df[cols["volume"]]
        else:
            data["Volume"] = 0.0
        return data[["Open","High","Low","Close","Volume"]].astype(float)
    # last resort if columns are already standard
    return df[["Open","High","Low","Close","Volume"]].astype(float)

def _user_agent_headers():
    # Some providers return empty unless a UA is set
    return {"User-Agent": "Mozilla/5.0 (compatible; PinnacleTrade/1.0)"}

# -----------------------------
# Yahoo + robust fallbacks
# -----------------------------
def _yahoo_fetch(pair: str, interval: str, period: str, debug: dict) -> pd.DataFrame:
    combos = [
        (interval, period),  # as requested
        ("1d", "1y"),
        ("1d", "2y"),
        ("1wk", "5y"),
    ]
    for iv, pr in combos:
        try:
            ...
            debug.setdefault("yahoo_attempts", []).append({"interval": iv, "period": pr, "rows": 0 if df is None else int(df.shape[0])})
...
# -----------------------------
# Alpha Vantage (optional, needs key)
# -----------------------------
...
def _alpha_vantage_fetch(pair: str, interval: str, period: str, api_key: str, debug: dict) -> pd.DataFrame:
    ...
    def _av_intraday(mins: str, outputsize: str = "compact") -> pd.DataFrame:
        ...
        params = dict(function="FX_INTRADAY", from_symbol=frm, to_symbol=to,
                      interval=f"{mins}min", outputsize=outputsize, apikey=api_key)
        r = requests.get(url, params=params, timeout=30, headers=_user_agent_headers())
        j = r.json()
        key = f"Time Series FX ({mins}min)"
        if key not in j:
            debug.setdefault("alpha_vantage_resp", []).append({"endpoint":"intraday", "interval":mins, "note":list(j.keys())[:5]})
            return pd.DataFrame()
        rec = []
    ...
    def _av_daily(outputsize: str = "compact") -> pd.DataFrame:
        url = "https://www.alphavantage.co/query"
        params = dict(function="FX_DAILY", from_symbol=frm, to_symbol=to,
    ...
    try:
        ...
        if interval == "1wk":
            ...
            if base.empty:
                return base
            return base.resample("W").agg({"Open":"first","High":"max","Low":"min","Close":"last","Volume":"sum"}).dropna()
        # default to daily
        return _av_daily(outputsize)
    except Exception as e:
...
# -----------------------------
# New: Stooq (free CSV) fallback
# -----------------------------
def _stooq_symbol(pair: str) -> str | None:
    """
    ...
    Examples:
      ...
      USDZAR=X -> usdzar
      GBPUSD=X -> gbpusd
    For stocks like AAPL, return None (we only use stooq for FX daily).
    """
    p = pair.upper().strip()
...
def _stooq_fetch(pair: str, interval: str, period: str, debug: dict) -> pd.DataFrame:
    """
    Stooq delivers **daily** FX CSV. Accept files with 4 or 6 columns:
    ...
    url = f"https://stooq.com/q/d/l/?s={sym}&i=d"  # daily only
    try:
        ...
        # Parse CSV leniently (some feeds omit Volume)
        ...
        # Stooq only gives daily. If the user requested weekly, resample here.
        if interval == "1wk" and not df.empty:
            df = df.resample("W").agg({"Open":"first","High":"max","Low":"min","Close":"last","Volume":"sum"}).dropna()

        # Record attributes so the UI shows what we actually used
        df.attrs["used_interval"] = "1d" if interval != "1wk" else "1wk"
...
# Provider dispatcher
# -----------------------------
def fetch_market_data(pair: str, interval: str, period: str, debug: dict | None = None) -> pd.DataFrame:
    if debug is None:
        debug = {}
    provider_order = []
    ...
    if provider == "alpha_vantage":
        ...
        if not df.empty:
            ...
            # stooq is daily — reflect that to UI
            df.attrs["used_interval"] = "1d"
            df.attrs["used_period"] = "5y"
            return df
    else:
        # yahoo first, then stooq, then alpha (if key provided)
        provider_order.append("yahoo")
        df = _yahoo_fetch(pair, interval, period, debug)
        if not df.empty:
            df.attrs["used_provider"] = "yahoo"
...
def signal(
    ...
    payload = {
        "pair": pair,
        "interval": getattr(df, "used_interval", interval),
        "period": getattr(df, "used_period", period),
...
def dashboard(
    ...
    ind = compute_indicators(df, ema_fast=ema_fast, ema_slow=ema_slow,
                             rsi_len=rsi_len, macd_fast=macd_fast, macd_slow=macd_slow, macd_signal=macd_signal).dropna()
    if ind.empty:
    ...
    <html><head>
      ...
      <div class="card">
        ...
        <div class="footer">If intraday keeps failing, set DATA_PROVIDER=alpha_vantage and ALPHA_VANTAGE_KEY in Render.</div>
